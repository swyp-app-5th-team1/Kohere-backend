...
> **개정(2026-09-08, [#314](https://github.com/swyp-app-5th-team1/Kohere-backend/issues/314))**: **번역 대상에서 추천 조정 제안이 빠졌다.** v1 추천 경로가 제거되면서 `suggestions.message`·`actions[].detail`을 쓰는 응답이 사라졌고 `diagnosisSuggestions` 컬렉션도 드롭했다 — 따라서 2026-06-24 개정의 (2)항과 **Decision 8 후반부(추천 제안 번역 영속)는 폐기**한다. 문항·선택지 번역 전략(인라인 언어-키 맵·`getLanguage` 동기 취득·`en` 폴백·`code` 불변)은 **그대로 유효**하며 서버 주도 흐름(`/start`·`/next`)과 퀴즈·생활 팁이 계속 쓴다. 문항 제공 경로는 v2로 옮겨 갔다([ADR-0028](./0028-diagnosis-questions-catalog-store.md) 같은 날짜 개정). 관련 유저 스토리는 **US-2-7**이다(US-2-6은 그 스토리에 흡수됐다).
