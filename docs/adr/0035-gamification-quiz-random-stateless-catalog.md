...
4. **i18n은 진단 방식을 재사용한다([ADR-0029](./0029-diagnosis-i18n-strategy.md)).** 문제 지문(`question`)·각 보기 텍스트(`choices[].text`)·해설(`explanation`)을 문항 도큐먼트 안에 **언어 코드를 키로 하는 맵**(`Map<lang, String>`, 예: `{ "en": …, "ja": …, "ko": … }`)으로 인라인 임베드한다. 표시 언어는 `user` 모듈 공개 query `getLanguage(userId)`(등록 국가 → `countries.lang`으로 도출, 미지원·미매핑이면 `en` 폴백, 에러 아님)로 취득한다. **보기 키 `A`~`D`는 언어 불변**이며 채점은 키로 수행한다(어느 언어로 표시돼도 같은 키로 채점).
