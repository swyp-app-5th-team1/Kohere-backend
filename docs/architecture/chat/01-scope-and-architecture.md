...
TEXT 전송 → 인증·검증 → 원문과 번역 대기 작업 저장 → Google 번역 결과 확정 → 수신자에게 원문·결과 함께 전달
```
...
9. Google 번역 지연·실패는 원문 저장과 발신자 ACK를 취소하지 않는다. 수신자는 최종 번역 상태가 정해진 뒤 원문과 결과를 한 이벤트로 받으며, 실패하면 원문을 표시한다.
