...
이 폴더는 관리자 기능, 자동 보존·물리 삭제와 iOS 채팅 푸시의 상세 설계·남은 운영 검증을 모아 둔다. 현재 사용자 채팅 API와 사용자별 논리 삭제는 상위 [채팅 설계 문서](../README.md)를 따른다.
...
| 채팅 푸시 | iOS FCM 기기 등록·삭제, 새 TEXT·문의 카드·신청 카드 발송 코드와 무효 토큰 정리 | Firebase/APNs 연결, 채팅방 딥링크 실제 기기 검증과 운영 보강 |
...
| [06-chat-notifications.md](06-chat-notifications.md) | iOS FCM 기기 토큰, notification/data payload, 새 TEXT·문의 카드·신청 카드 푸시, 채팅방 딥링크와 단계별 구현 계획 |

iOS 앱에서 기기 등록·삭제 API와 푸시 수신·클릭 처리를 연동할 때는 상위 [11-frontend-push-notification-guide.md](../11-frontend-push-notification-guide.md)를 먼저 읽는다.
