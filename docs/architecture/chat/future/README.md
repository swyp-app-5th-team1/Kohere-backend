...
이 폴더는 지금 바로 구현하지 않을 관리자 기능과 자동 보존·물리 삭제 기능을 모아 둔다. 현재 사용자 채팅 API와 soft delete는 상위 [채팅 설계 문서](../README.md)를 따른다.

## 현재와 후속 범위

| 구분 | 현재 구현 | 후속 고도화 |
| --- | --- | --- |
| 사용자 삭제 | 내 목록과 과거 이력 숨김, 짧은 Undo | 삭제 후 3개월 만료 확정 |
| DB 메시지 | 원문·번역본 유지 | 양쪽 모두 만료된 공통 범위 물리 삭제 |
...
위 조건이 준비되기 전에는 상위 문서의 soft delete 데이터나 신고 evidence를 임의로 물리 삭제하지 않는다.
