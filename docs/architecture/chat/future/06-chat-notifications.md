...
> 상태: 구현 전 계획
>
> 범위: iOS 앱의 1:1 채팅 `TEXT` 메시지 FCM 푸시와 채팅방 딥링크

이 문서에서 말하는 기능은 예약 시각에 울리는 `alarm`이 아니라 새 채팅을 알려 주는 `notification`, 즉 **알림**이다.

현재 채팅은 MySQL 저장, REST 이력 조회, STOMP 실시간 전달을 제공한다. 하지만 앱이 백그라운드이거나 종료되면 WebSocket 연결만으로 사용자를 알릴 수 없다. 이 문서는 새 `TEXT` 메시지가 저장됐을 때 FCM과 APNs를 거쳐 iOS 상단 알림을 표시하고, 사용자가 알림을 누르면 해당 채팅방으로 이동하는 기능을 정의한다.
...
| 발송 대상 | 상대방이 새로 저장한 `TEXT` 메시지 |
| 제외 메시지 | `INQUIRY_CARD`, `BOOKING_CARD`, 중복 재전송 |
| 외부 전달 경로 | 백엔드 → FCM → APNs → iOS |
| 발송 시점 | 채팅 메시지 트랜잭션 커밋 이후 |
| 수신자 | 발신자가 아닌 채팅방 상대 참여자 |
| 여러 기기 | 수신자의 등록된 활성 iOS 기기 모두 발송 |
| 표시 제목 | `CHAT_MESSAGE`이면 `채팅` |
| 표시 본문 | `"{listingTitle}"으로부터 새 메시지가 도착했어요` |
| 표시 시각 | 원래 `TEXT`가 MySQL에 저장된 `sentAt` |
...
`type`은 현재 `CHAT_MESSAGE` 한 값만 사용한다. 향후 다른 종류의 푸시가 추가되더라도 iOS 앱이 안전하게 분기할 수 있도록 payload에 명시한다.

FCM이나 iOS가 `CHAT_MESSAGE`를 `채팅`으로 자동 변환하지는 않는다. 백엔드가 같은 채팅 이벤트 값을 바탕으로 `notification`과 `data`를 각각 완성한다.
...
- `INQUIRY_CARD`와 `BOOKING_CARD` 푸시
- 메시지 원문·번역문 미리보기
...
```mermaid
flowchart LR
    SENDER[발신자 iOS 앱] -->|STOMP TEXT| CHAT[Chat 모듈]
...
새 `TEXT` 메시지를 실제로 저장한 경우에만 chat 모듈이 `ChatTextMessageCreatedEvent`를 발행한다.
...
`sentAt`은 FCM 요청을 시도한 시각이 아니라 원래 `TEXT`가 MySQL에 처음 저장된 시각이다. 외부 장애로 이벤트를 재처리하더라도 이 값은 바뀌지 않아 앱의 `2시간 전` 같은 표시가 재시도 시점으로 밀리지 않는다.
...
- `title`: `채팅`
- `body`: `"{listingTitle}"으로부터 새 메시지가 도착했어요`

앱이 종료돼 있어도 APNs가 이 영역을 사용해 OS 알림을 표시한다.

백엔드는 `data`를 FCM에 먼저 보내고 FCM이 `notification`을 생성하게 하지 않는다. 채팅 이벤트에서 `type`과 `listingTitle`을 읽어 `notification.title`, `notification.body`, `data`를 한 번에 조립한 뒤 완성된 payload를 FCM으로 보낸다.
...
| `sentAt` | 예 | 원래 `TEXT`가 서버에 저장된 UTC 시각과 상대 시간 표시 |
...
- notification listener 실패는 기존 Event Publication Registry의 미완료 이벤트로 확인하고 재처리할 수 있어야 한다.
...
| 일시적 provider 오류 | 제한적으로 재시도하고 실패 로그 기록 |
...
- `listingTitle`을 사용해 notification body 조립
- data의 여섯 필드가 모두 문자열
...
| 2. 채팅 이벤트 | 신규 `TEXT` 커밋 후 수신자 알림 후보 이벤트 발행 | 중복·실패·발신자 제외 테스트 통과 |
| 3. FCM 발송 | Firebase sender, payload 조립, 토큰별 결과·무효 토큰 정리 | fake adapter 테스트와 dev 발송 성공 |
...
Firebase 초기 설정은 이미 준비됐다. 다음 백엔드 개발은 아래 두 PR로 나누는 것을 기본으로 한다.
...
### 13.2 PR 2: 채팅 이벤트와 FCM 발송

- chat 모듈에 `ChatTextMessageCreatedEvent`를 추가한다.
- `ChatTextMessageService`가 신규 `TEXT`를 저장한 경우에만 이벤트를 발행한다.
- notification 모듈의 listener가 수신자 기기들을 조회한다.
- `PushMessageSender` port와 Firebase/no-op 구현을 추가한다.
- 백엔드가 `type`, `listingTitle`, `sentAt`으로 최종 notification/data payload를 조립한다.
...
- [x] `CHAT_MESSAGE → title=채팅`과 `listingTitle` 기반 body 문구 확정
