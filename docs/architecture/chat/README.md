...
> 상태: 단계별 구현 진행 중
>
> 현재 구현: 채팅방 생성·조회·사용자별 삭제·상대 차단, 메시지 이력, STOMP 인증·구독, TEXT·BOOKING_CARD 실시간 전달
...
| [future/README.md](future/README.md) | 관리자 신고 처리·삭제 복구, 3개월 만료, 물리 삭제 등 후속 고도화 설계 |
...
| 메시지 종류 | 사용자가 보내는 `TEXT`, 신청 완료 때 서버가 만드는 `BOOKING_CARD` |
...
- 그룹 채팅
- 이미지·영상·파일 메시지
- `LISTING_CARD`, `SYSTEM` 메시지 (`BOOKING_CARD`는 이번 범위에 포함)
