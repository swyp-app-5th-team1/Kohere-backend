...
응답의 `myRole`은 앱이 신청 카드를 임차인용 또는 임대인용으로 표시할 때 사용한다. `canApply`는 임차인 화면 상단의 Apply now 배너 노출 여부를
서버가 계산한 값이다.
...
`canApply=true`는 현재 사용자가 임차인이고, 이 매물에 기존 신청 기록이 없으며, 차단 관계가 없고, 현재 공개 매물에 활성 방 상품이 하나 이상 있다는 뜻이다.
이미 해당 매물의 방 상품 중 하나에 신청했거나, 임대인이 조회하거나, 차단·매물 비공개·활성 방 상품 없음 상태라면 false다. 예약 목록에서 숨긴 신청도 취소된
것이 아니므로 기존 신청으로 센다.

이 값은 배너 노출을 위한 사전 판단이다. 실제 신청 API는 사용자가 선택한 `roomOfferId`, 입주 희망일과 계약기간을 포함한 전체 규칙을 다시 검증한다. 기존
DB 중복 규칙인 `(tenant_id, room_offer_id)`는 변경하지 않는다.
