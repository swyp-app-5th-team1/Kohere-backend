...
- 세입자가 `POST /api/v1/listings/{listingId}/bookings`로 `moveInDate`·`contractPeriod`를 보내면 **booking 모듈**이 **listing 모듈**에 매물 조회·공개 검증을 동기 호출(`->>`)해 임대인·매물 정보를 받은 뒤 `Booking`을 `REQUESTED`로 **저장소(추후 결정)**에 생성한다. 매물 조회는 listing 모듈이 MongoDB에서, 예약 저장은 booking 모듈이 저장소(추후 결정)에서 각자 자기 데이터를 읽고 쓴다.
