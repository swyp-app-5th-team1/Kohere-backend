...
sequenceDiagram
    ...
    Note over LIST: bbox/반경 모드 상호 배타 검증<br/>location 2dsphere + roomOffers 필터 적용<br/>좌표 그리드로 마커 집계<br/>비클러스터는 결과 상한(예 500건) 검사
    alt 정상 클러스터 조회
        LIST->>DB: 지도 bbox 매물 조회·클러스터 집계<br/>(location 2dsphere, status=PUBLISHED)
...
- 지도 패닝 시 `GET /api/v1/listings/map`을 호출해 `listing` 모듈이 MongoDB `location`의 `2dsphere` 인덱스로 bbox(또는 center+radius) 영역의 공개 건물 매물을 조회하고, 가격·조건 필터는 `roomOffers[]` 기준으로 적용한 뒤 줌 레벨 기준 클러스터로 집계한다(`200 OK` + `data.clusters[]`/`total`).
