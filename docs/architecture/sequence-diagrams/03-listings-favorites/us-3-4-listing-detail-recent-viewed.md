...
sequenceDiagram
    ...
    alt 존재하는 매물
        Note over LIST: 상세 구성 (직접 연락처 미노출)<br/>건물 공통정보 + roomOffers[] 방 상품<br/>로그인 시 (userId, listingId) 유니크로<br/>최근 본 매물 upsert, viewedAt 갱신<br/>비로그인은 favorited=false, 기록 안 함
...
- `GET /api/v1/listings/{listingId}`로 `listing` 모듈이 MongoDB에서 건물 공통 상세(`imageUrls[]`·`type`·`location`·`address`·`propertyPolicies`·`facilities`·`featureSummary`)와 방 상품 목록(`roomOffers[]`: 가격·계약·재고·성별·필터 태그)을 조회한다. 직접 연락처는 노출하지 않고 신청·문의는 채팅으로 연결한다(인증 선택이라 SEC 없이 `listing` 모듈로 직접 요청).
