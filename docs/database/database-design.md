...
| [`diagnosis`](#4-4-diagnosis) | **MongoDB** | `diagnoses`(확정 결과)·`diagnosisQuestions`(문항·선택지 카탈로그)·`diagnosisFlowSessions`(서버 주도 진행 세션) — 인라인 언어-키 맵 번역, US-2-7 | ✅ |
...
- **진행 중 저장(server-stateful)**: 진행 답은 이 컬렉션이 아니라 `diagnosisFlowSessions`에 담긴다 — `POST /api/v2/diagnoses/next`가 현재 문항 답을 세션 draft에 채우고, 마지막 슬롯을 답하면 서버가 재검증해 **정본 진단을 이 컬렉션에 `COMPLETED`로 저장**한다(`submittedAt` 설정). 따라서 이 컬렉션에는 **종료 상태(`COMPLETED`·`DISCARDED`)만** 쌓인다. **이력·최근·상세 조회는 `COMPLETED`만 노출**한다.
...
- **게스트 문서를 만드는 경로는 v2 흐름뿐**: **진단 조회 3종은 회원 전용으로 유지한다** — SecurityConfig에 그 경로용 permitAll 매처를 두지 않아 현행대로 `.anyRequest().authenticated()`에 남는다(토큰 없으면 401). 게스트 진단의 정본 경로는 v2뿐이고(`POST /api/v2/diagnoses/start` → `POST /api/v2/diagnoses/next` → `GET /api/v2/diagnoses/{id}/recommendations`), 신규 permitAll 매처의 대상도 `/api/v2/diagnoses/**`뿐이다. 귀결로 **게스트용 이력·최근 시맨틱을 정의할 필요가 없다**(게스트는 401이라 도달하지 못하고, 세션 키가 `/start`마다 새로 발급돼 한 키에 달리는 종료 진단이 최대 1건이라 이력 개념이 성립하지 않는다). 이 컬렉션의 게스트 문서는 v2가 **종료 상태로만** 쓴다 — 확정(`COMPLETED`)과 지역 0건 폐기(`DISCARDED`)뿐이라 게스트 문서에는 `IN_PROGRESS`가 없다(v2 진행 중 상태는 `diagnosisFlowSessions`에 있다, 아래).
...
- **문항·선택지 출처(US-2-7)**: 문항 제공은 **서버 주도 질의응답**이다 — 클라이언트가 `POST /api/v2/diagnoses/start`·`/next`를 호출하면 서버가 (카탈로그 + 진행 중 진단에 저장된 답 + 사용자 언어 키)으로 **그 step 질문 1개만** 선정해 `{ step, field, question(사용자 언어 라벨 문자열), select{type,max}, options[{code,label}] }`로 내려준다(한 번에 다 주지 않음, 다음 문항은 서버가 정한다). 현재 step 답은 별도로 `POST /api/v2/diagnoses/next`(body `{ field, code }`; `conditions`처럼 다중은 `codes` 배열)로 보내면 서버가 진행 중(`IN_PROGRESS`) 진단에 저장한다(누적 답 묶음 전송 없음). 흐름은 `POST /start → POST /next × 6`이며, 마지막 슬롯 답이 오면 서버가 **자동 확정**한다. 반환 선택지 `code`는 **확정 검증 enum과 1:1 동일 출처**다(언어 무관 단일 키). 문항·선택지 카탈로그는 **MongoDB `diagnosisQuestions` 컬렉션**(아래)에 데이터로만 영속한다. **분기는 서버 비즈니스 로직(diagnosis 서비스 코드)이 결정한다(클라 로컬 분기·카탈로그 분기 메타 아님)** — ③(step 3)은 저장된 `purpose`에 따라 서비스가 알맞은 질문만 담는다(`STUDY`→대학 질문 `university`, `NON_STUDY`→지역구 질문 `district`; 한 응답에 두 목록을 함께 주지 않음). 잘못된 현재 step 답(미정의 enum, 목적-대학/지역 불일치 등)은 공통 `400 INVALID_INPUT`+`errors[]`.
- **라벨 번역(US-2-7)**: 표시 `question`·`label`만 사용자 표시 언어로 번역하고 `code`는 언어 무관 동일이다. 번역 표시 문자열은 `diagnosisQuestions` 도큐먼트 내부 `question`·`label`의 **인라인 언어-키 맵**(`{ "en": "...", "ja": "...", "ko": "..." }`)에 임베드하고, **해당 언어 키가 없으면 영어(`en`) 폴백**(에러 아님, `Accept-Language` 비의존). 표시 언어는 **user 모듈 공개 query(`getLanguage`)로 동기 취득**하고 `user`가 `users.lang`(사용자가 고른 표시 언어)이 있으면 그 값, 없으면 `en`으로 폴백한다(토큰 클레임 분기 아님, [ADR-0002](../adr/0002-inter-module-communication-via-events.md) Decision 5 · [ADR-0029](../adr/0029-diagnosis-i18n-strategy.md) 개정(#141); 교차 모듈 no-FK 값 참조) → 모듈 의존 `diagnosis`→`user`.
...
진단 6단계의 문항·선택지·제약을 **데이터로만** 영속하는 카탈로그 컬렉션이다(US-2-7). 분기 메타(`branchOn` 등)는 두지 않는다 — 어느 질문을 낼지(③ 대학/지역)는 diagnosis 서비스 비즈니스 로직이 결정한다(D4). v2 서버 주도 흐름(#157 · [ADR-0036](../adr/0036-diagnosis-v2-server-driven-flow.md))이 ① 지역 0건일 때 끼워 넣는 **예외질문도 서버 코드에 하드코딩한 합성 문구가 아니라 이 카탈로그의 일반 문항**(step 1·`field=regionRetry`)이다 — 예외질문을 따로 관리하지 않고 6단계 문항과 같은 곳에 둔다(아래 註). 표시 문자열(번역)은 별도 컬렉션 없이 도큐먼트 내부 `question`·`label`의 **인라인 언어-키 맵**(`{ "en": "...", "ja": "...", "ko": "..." }`)에 임베드한다(US-2-7). 서버 주도 흐름(`/start`·`/next`)이 (이 카탈로그 + 진행 중 진단에 저장된 답 + 사용자 언어 키)으로 **그 step 질문 1개만** 선정·조립해 내려준다(한 번에 다 주지 않음, 다음 문항은 서버가 정한다). 선택지 `code`는 **확정 검증 enum과 동일 출처**(언어 무관 단일 키)다. 시드(정본 시드 JSON 주입)로 적재, 운영 중 `active`로 가변.
...
- **③ 분기(서버 결정)**: 대학/지역 단계는 **분기 메타 없이** 두 질문(`university`·`district`)이 데이터로 각각 존재하고, `/next`가 호출되면 **diagnosis 서비스 비즈니스 로직**이 진행 중 진단에 저장된 `purpose`를 보고 어느 질문을 낼지 결정해 하나만 골라 내려준다 — `STUDY`면 대학 목록으로 `university` 질문, `NON_STUDY`면 지역구 목록으로 `district` 질문(한 응답에 두 목록을 함께 주지 않음, 클라 로컬 분기 아님).
...
11. **진단 데이터 보존·파기(#181·#314)**: 게스트 문서는 v2 흐름에서만 생성된다 — 조회 3종은 회원 전용이라 게스트가 도달하지 못한다(§4-4). **미확정 항목**: 확정 진단(`COMPLETED`)에 TTL이 없고 탈퇴도 이 컬렉션을 지우지 않아 영구 보존된다 — 보존 기간과 파기 주체를 정해야 한다.
