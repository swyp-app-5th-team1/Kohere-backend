...
| GET | `/api/v2/diagnoses/{diagnosisId}/recommendations/map` | 필수 | 소유권 검증 키 |
...
| GET | `/api/v2/diagnoses/{diagnosisId}/recommendations/map` | 확정 진단의 추천 매물 지도 마커 전체(페이지 없음, 상한 500건) | 선택(게스트 허용) | 200 |
...
> **경로를 가른 이유는 문서 생성기 제약이다.** 같은 경로에 쿼리 파라미터로 표현을 나누면(`?view=map`) 두 응답이 같은 `(path, method, 200)`을 공유해 OpenAPI 스키마 중 하나가 조용히 버려진다([ADR-0017](../../adr/0017-openapi-swagger-ui-from-restdocs.md) 병합 규칙). 취향 문제가 아니므로 되돌리지 말 것.

- **인증**: 선택(게스트 허용). v2-3과 같은 소유권 규칙 — 타인 `403 FORBIDDEN`, 게스트는 `X-Guest-Session-Id`가 필수이며 회원↔게스트 교차 조회는 양방향 403이다.
- **확정(`COMPLETED`) 진단만 조회된다** — 폐기 기록과 미완주 초안은 `404 DIAGNOSIS_NOT_FOUND`다. 미완주 초안은 조건이 비어 있어 그대로 매칭하면 "조건 없는 전체 매물"로 붕괴하는데, 이 경로는 페이지 상한을 걷어내므로 그 붕괴가 곧 전량 조회가 된다. **상태 검사(404)가 소유권 검사(403)보다 먼저 돈다.**
- **정렬을 받지 않는다.** 상한에 걸려 잘릴 때 남는 집합은 추천 기본 정렬(찜 수·최근 수정) 상위다 — `sort=price`로 본 v2-3 목록과 **다른 집합일 수 있다**.
...
// 매칭 있음
{
  ...
  "data": {
    "markers": [
      ...
      { "listingId": "6858e2000000000000000002", "lat": 37.497942, "lng": 127.027621 }
    ],
    "total": 137
  },
  "error": null
...
```jsonc
// 매칭 0건 — 에러가 아니다
{ "success": true, "data": { "markers": [], "total": 0 }, "error": null }
```

- `markers`: 조건에 맞는 매물의 지도 좌표. **최대 500건**이다.
- `total`: 조건에 맞는 **전체** 매물 수(절단 전). `markers` 길이보다 크면 서버 상한에 걸려 일부만 실린 것이다.

> **상한을 넘으면 에러가 아니라 잘라서 준다.** 매물 지도 조회는 범위를 좁힐 수단(bbox)이 있어 `400`으로 끊지만, 진단은 조건이 고정이라 사용자가 좁힐 방법이 없다 — 복구할 수 없는 에러를 주는 대신 상한까지 채워 보낸다.
>
> `total`은 `count`와 `find`가 별개 쿼리라 경계에서 1~2건 오차가 날 수 있는 **참고값**이다.
