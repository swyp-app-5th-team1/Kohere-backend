 ...
 * 별도 전송한다. BOOKING_CARD는 프런트엔드 SEND가 아니라 서버가 신청 이벤트를 검증한 뒤 생성한다.
 */
public record ChatMessageCreatedPayload(
    ...
    Long messageId,
    /** TEXT를 만든 프런트엔드 UUID. 서버 생성 BOOKING_CARD에는 null이다. */
    UUID clientMessageId,
    /** 메시지가 속한 채팅방 식별자. */
    Long chatRoomId,
    /** TEXT 발신 사용자 ID. 서버 생성 BOOKING_CARD에는 null이다. */
    Long senderId,
    /** TEXT 또는 서버 생성 BOOKING_CARD를 구분한다. */
    MessageType type,
    /** 모든 참여자에게 동일하게 전달하는 TEXT 원문. BOOKING_CARD에는 null이다. */
    String originalContent,
    /** 서버가 신청 정보로 만든 카드 payload. TEXT에는 null이다. */
