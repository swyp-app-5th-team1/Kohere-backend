...
public class ListingCatalogLabelFieldChangeUnit {
  ...
  public void execution(MongoTemplate mongo) {
    if (!mongo.collectionExists(ListingCatalogDocument.COLLECTION_NAME)) {
      return;
    }

    MongoCollection<Document> catalog = mongo.getCollection(ListingCatalogDocument.COLLECTION_NAME);
