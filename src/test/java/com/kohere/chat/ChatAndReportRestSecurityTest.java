 ...
 *
 * <p>방 참여자 여부, 본인 매물 문의 금지, 신고 대상 검증 같은 객체 단위 권한은 각 기능의 서비스 테스트에서 다룬다. 여기서는 채팅 REST에 공통인 "온보딩을 완료한
 * 사용자" 조건만 검증한다.
 */
@WebMvcTest(
    controllers = {
      ...
      ChatRoomReportController.class
    })
...
class ChatAndReportRestSecurityTest {
  ...
  private static final String CHAT_ROOM_REPORT_PATH = "/api/v1/chat-rooms/556/reports";
  ...
  // 신고 저장과 증거 캡처는 전용 서비스·통합 테스트가 검증하고 여기서는 채팅방 신고 POST의 ROLE_USER 경계만 확인한다.
  ...
  /** 채팅방 신고도 인증 여부만 보는 기본 규칙으로 빠지지 않고 온보딩 임시 권한을 403으로 차단하는지 검증한다. */
  @Test
  @DisplayName("채팅방 신고 REST: ROLE_ONBOARDING은 403")
  void reports_withOnboardingRole_returnsForbidden() throws Exception {
    mockMvc
        .perform(
            post(CHAT_ROOM_REPORT_PATH)
  ...
  /** 정식 사용자 요청은 신고 컨트롤러까지 도달하고 신규 접수의 201 응답을 받는다. */
  @Test
  @DisplayName("채팅방 신고 REST: ROLE_USER는 컨트롤러까지 통과")
  void report_withUserRole_reachesController() throws Exception {
    ...
    Report report =
        Report.builder()
            ...
            .updatedAt(now)
            .build();
    given(reportService.createChatRoomReport(7L, 556L, ReportReason.SPAM))
        .willReturn(new ReportCreationResult(report, true));
    ...
    mockMvc
        .perform(
            post(CHAT_ROOM_REPORT_PATH)
                ...
                .content("{\"reason\":\"SPAM\"}"))
        .andExpect(status().isCreated())
        .andExpect(jsonPath("$.data.reportId").value(15L));
  }
}
