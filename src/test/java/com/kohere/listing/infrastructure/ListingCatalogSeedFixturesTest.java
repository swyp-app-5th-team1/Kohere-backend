...
class ListingCatalogSeedFixturesTest {
  ...
  void documents_기존코드의_영어라벨이_Figma문구와_일치한다() {
    Map<String, ListingCatalogDocument> documents = documentsById();
  ...
  private static Map<String, ListingCatalogDocument> documentsById() {
    return ListingCatalogSeedFixtures.documents().stream()
        .collect(Collectors.toMap(ListingCatalogDocument::getId, document -> document));
  }
}
